...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const determineRole = (roles: ('STUDENT' | 'ADMIN' | 'INSTRUCTOR')[] | string[]): UserRole => {
    if (!roles || !Array.isArray(roles) || roles.length === 0) {
      console.warn('No roles provided or roles is not an array:', roles);
      return null;
    }
    ...
    console.warn('Unknown role values:', roles);
    return null;
  };
  ...
  const login = async (email: string, password: string): Promise<{ success: boolean; error?: string; role?: UserRole }> => {
    try {
      const apiBaseUrl = import.meta.env.VITE_API_URL || import.meta.env.VITE_API_BASE_URL || 'http://localhost:3001/api';
