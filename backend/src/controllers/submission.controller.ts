...
class SubmissionController {
  ...
  submitAssignment = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      res.status(500).json({ error: 'Failed to submit assignment', details: error instanceof Error ? error.message : String(error) });
    }
  };
  ...
  submitProject = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      res.status(500).json({ error: 'Failed to submit project', details: error instanceof Error ? error.message : String(error) });
    }
  };
