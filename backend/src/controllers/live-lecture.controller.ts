...
import { PrismaClient, UserRole, LiveLectureBatchType } from '@prisma/client';
...
export const liveLectureController = {
  ...
  createModule: async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      res.status(500).json({ error: 'Failed to create module', details: error instanceof Error ? error.message : String(error) });
    }
  },
