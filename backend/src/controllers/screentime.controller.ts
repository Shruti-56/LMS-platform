...
export class ScreenTimeController {
  ...
  recordPing = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const screenTime = await prisma.screenTime.upsert({
        ...
        update: {
          totalSeconds: { increment: 30 }, // Add 30 seconds per ping
          lastPing: new Date(),
        },
        create: {
          userId,
      ...
      res.status(500).json({ error: 'Failed to record screen time', details: error instanceof Error ? error.message : String(error) });
    }
  };
  ...
  getMyScreenTime = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      res.status(500).json({ error: 'Failed to get screen time', details: error instanceof Error ? error.message : String(error) });
    }
  };
}
